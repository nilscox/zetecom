...
const Carousel: React.FC<CarouselProps> = ({ width, delay, slides }) => {
  ...
  return (
    <div className="carousel" style={{ width }}>
      <svg viewBox="0 0 32 32" className="prev-slide" onClick={() => setSlide(s => (s - 1 + slides.length) % slides.length)}>
      ...
      <svg viewBox="0 0 32 32" className="next-slide"  onClick={() => setSlide(s => (s + 1) % slides.length)}>
